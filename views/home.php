...
<section class="sec sec-dark" id="sismos">
  <div class="wrap">
    ...
    <div class="rt-monitor-hero">
      ...
      <div class="shake-viz">
        <div class="shake-status" id="shakeStatus">✓ SACUDIDA LEVE</div>
    ...
    <div class="seismo-layout">
      <div>
        <div class="sg-main-card">
          <div class="phdr">
            <div style="width:32px;height:32px;background:rgba(255,77,26,.15);border-radius:var(--rs);display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0">🌋</div>
            <div>
          ...
          <div class="sg-controls">
            ...
            <button class="sg-reset" id="sgReset">↺ Reiniciar</button>
        ...
        <div style="margin-top:14px;background:var(--card);border:1px solid var(--border2);border-radius:var(--r);overflow:hidden">
          <div class="phdr"><span class="ldot"></span>Sismos Recientes — USGS API <button class="sg-reset" id="refreshQ" style="margin-left:auto;padding:4px 11px">↺</button></div>
...
<section class="sec" id="placas">
  <div class="wrap">
    ...
    <div class="plates-grid">
      ...
      <div class="plate-card">
        <div class="pc-visual"><div class="wave-rings"><div class="wr"></div><div class="wr"></div><div class="wr"></div><div class="wr"></div><div class="wc-star">★</div></div></div>
    ...
    <div class="arduino-layout">
      <div>
        <div class="ard-panel" style="margin-bottom:14px">
          <div class="phdr"><span class="ldot"></span>Sismógrafo Arduino — Señal en Tiempo Real</div>
        ...
        <div class="ard-panel">
          ...
          <div class="what-todo"><div class="wt-lbl">¿Qué hacer?</div><div class="wt-steps" id="wtSteps"><div class="wts safe"><span class="wts-icon">✓</span>El sistema monitorea continuamente.</div><div class="wts safe"><span class="wts-icon">✓</span>En caso de alerta, sigue los protocolos de evacuación.</div></div></div>
          <div class="ard-log" id="ardLog"><div class="ald i"> Arduino UNO iniciado — Sensor MPU-6050 conectado</div><div class="ald i">📡 Esperando señal de vibración…</div></div>
          <div class="demo-note"> <span>Sistema demostrativo educativo. Simula el comportamiento de un sensor MPU-6050 real. En producción, los datos vendrían del Arduino por puerto serial.</span></div>
          <div class="ard-controls"><button class="btn-acc" id="ardSimBtn">▶ Simular Alerta</button><button class="btn-out" id="ardResetBtn">↺ Reiniciar</button></div>
        </div>
      </div>
      <div class="ard-info">
        <div class="ard-info-card"><div class="aic-title"> Componentes del Sistema</div><div class="aic-row"><div class="aic-icon">🖥</div><div><strong style="display:block;font-size:.82rem;color:var(--text);margin-bottom:2px">Arduino UNO R3</strong><span>Microcontrolador principal. Procesa señales del sensor a 16MHz.</span></div></div><div class="aic-row"><div class="aic-icon">📐</div><div><strong style="display:block;font-size:.82rem;color:var(--text);margin-bottom:2px">Sensor MPU-6050</strong><span>Acelerómetro + giroscopio. Detecta vibración en 3 ejes (X, Y, Z).</span></div></div><div class="aic-row"><div class="aic-icon">🔔</div><div><strong style="display:block;font-size:.82rem;color:var(--text);margin-bottom:2px">Buzzer Piezoeléctrico</strong><span>Emite alerta sonora cuando se supera el umbral de vibración.</span></div></div><div class="aic-row"><div class="aic-icon">💡</div><div><strong style="display:block;font-size:.82rem;color:var(--text);margin-bottom:2px">LED RGB de Alerta</strong><span>Verde (normal), Amarillo (precaución), Rojo (alerta sísmica).</span></div></div></div>
  ...
  <div class="wrap">
    ...
    <div class="map-container">
      <div class="map-ctrl-bar">
        <button class="mc-btn on" data-layer="seismic">🔴 Zona Sísmica</button>
        <button class="mc-btn" data-layer="volcanic">🟠 Volcán Activo</button>
        <button class="mc-btn" data-layer="flood">🔵 Riesgo Tsunami</button>
        <button class="mc-btn" data-layer="slides">🟡 Deslizamientos</button>
        <button class="mc-btn" data-layer="safe">🟢 Zona Segura</button>
        <button class="mc-btn" data-layer="quakes">📍 Sismos USGS</button>
        <button class="mc-btn" data-layer="all">⬤ Todo</button>
      </div>
...
<section class="sec sec-dark" id="clima">
  <div class="wrap">
    ...
    <div class="weather-bottom">
      <div class="solar-card"><div class="sc-hdr">Recorrido Solar — San Salvador</div><div class="sc-canvas-wrap"><canvas id="sunArc"></canvas></div><div class="sc-sunboxes"><div class="sc-sunbox"><div class="sc-sunbox-emoji">🌅</div><div class="sc-sunbox-time" id="sunriseBig">—</div><div class="sc-sunbox-lbl">Amanecer</div></div><div class="sc-sunbox"><div class="sc-sunbox-emoji">🌆</div><div class="sc-sunbox-time" id="sunsetBig">—</div><div class="sc-sunbox-lbl">Atardecer</div></div></div><div class="sc-dur" id="sunDur">—</div></div>
    ...
    <div class="moon-layout">
      <div class="moon-vis-card">
        <div class="moon-phase-selector" id="moonPhaseSelector">
          <button class="mps-btn" data-phase="0"><span>🌑</span><span class="mps-lbl">Nueva</span></button>
          <button class="mps-btn" data-phase="1"><span>🌒</span><span class="mps-lbl">Creciente</span></button>
          <button class="mps-btn" data-phase="2"><span>🌓</span><span class="mps-lbl">Cuarto Crec.</span></button>
          <button class="mps-btn" data-phase="3"><span>🌔</span><span class="mps-lbl">Gibosa Crec.</span></button>
          <button class="mps-btn" data-phase="4"><span>🌕</span><span class="mps-lbl">Llena</span></button>
          <button class="mps-btn" data-phase="5"><span>🌖</span><span class="mps-lbl">Gibosa Men.</span></button>
          <button class="mps-btn" data-phase="6"><span>🌗</span><span class="mps-lbl">Cuarto Men.</span></button>
          <button class="mps-btn" data-phase="7"><span>🌘</span><span class="mps-lbl">Menguante</span></button>
        </div>
      ...
      <div class="moon-right">
        <div class="tide-graph-card">
          ...
          <div class="tide-info"><div class="ti-item"><div class="ti-icon"></div><div class="ti-name">Mareas Vivas</div><div class="ti-desc">Luna nueva y llena. Máxima diferencia entre marea alta y baja.</div></div><div class="ti-item"><div class="ti-icon">🌬</div><div class="ti-name">Mareas Muertas</div><div class="ti-desc">Cuartos creciente y menguante. Menor variación de nivel.</div></div><div class="ti-item"><div class="ti-icon">🐟</div><div class="ti-name">Pesca Activa</div><div class="ti-desc">Mayor actividad pesquera en mareas altas con luna favorable.</div></div><div class="ti-item"><div class="ti-icon">⚠</div><div class="ti-name">Riesgo Costero</div><div class="ti-desc">Mareas vivas + lluvia = alto riesgo de inundación costera.</div></div></div>
        </div>
        ...
        <div class="moon-facts"><div class="mf-title">Datos Astronómicos Actuales</div><div class="mf-row"><span></span><span>Ciclo sinódico: <strong>29.53 días</strong> — El tiempo de luna nueva a luna nueva.</span></div><div class="mf-row"><span>🌍</span><span>Distancia media Luna-Tierra: <strong>384,400 km</strong> — varía ±5.5% en su órbita elíptica.</span></div><div class="mf-row"><span>🌊</span><span>Las mareas del Pacífico varían hasta <strong>2.4 metros</strong> durante mareas vivas en luna llena.</span></div></div>
      </div>
    </div>
  </div>
</section>
        ...
        <div class="evac-card" style="margin-bottom:16px">
          <div class="evac-svg-wrap">
            <svg viewBox="0 0 400 220" preserveAspectRatio="xMidYMid slice">
              ...
              <text x="125" y="52" text-anchor="middle" fill="white" font-size="11">⛰</text>
              <circle cx="215" cy="36" r="13" fill="rgba(0,212,176,.18)" stroke="var(--teal)" stroke-width="1.5"/>
              <text x="215" y="40" text-anchor="middle" fill="white" font-size="11">⛰</text>
              <circle cx="318" cy="58" r="13" fill="rgba(0,212,176,.18)" stroke="var(--teal)" stroke-width="1.5"/>
              <text x="318" y="62" text-anchor="middle" fill="white" font-size="11">⛰</text>
    ...
    <div class="tsunami-interactive">
      ...
      <div class="ts-evacuation-sim">
        ...
        <div class="ts-evac-controls">
          ...
          <button class="ts-evac-btn" onclick="resetEvacSim()">↺ Reiniciar</button>
...
<section class="sec" id="prevencion">
  <div class="wrap">
    ...
    <div style="margin-top:28px">
      ...
      <button class="bp-reset-btn" onclick="resetBP()">↺ Reiniciar checklist</button>
    </div>
  </div>
</section>
...
<section class="sec sec-dark" id="trivia">
  <div class="wrap">
    ...
    <div class="trivia-inner">
      <div class="tri-sel" id="triSel"><h3>Selecciona tu nivel de dificultad:</h3><div class="tri-lv-row"><button class="tlv" data-lv="easy">🟢 Básico</button><button class="tlv" data-lv="medium">🟡 Intermedio</button><button class="tlv" data-lv="hard">🔴 Avanzado</button></div></div>
...
<section class="sec sec-dark" id="recursos">
  <div class="wrap">
    ...
    <div class="res-g">
      <div class="rc card"><div class="rc-ico"></div><h4>Guía de Preparación Familiar</h4><p>Plan de emergencia completo con lista de verificación y contactos.</p><button class="rc-btn" onclick="fd('Guia_Familiar_NDA.pdf')">⬇ Descargar PDF</button></div>
      <div class="rc card"><div class="rc-ico"></div><h4>Plan de Evacuación Escolar</h4><p>Protocolo para docentes: rutas, puntos de reunión y comunicación.</p><button class="rc-btn" onclick="fd('Plan_Evacuacion_Escolar.pdf')">⬇ Descargar PDF</button></div>
      <div class="rc card"><div class="rc-ico"></div><h4>Material Didáctico — Primaria</h4><p>Fichas y actividades para enseñar prevención a niños de 6–12 años.</p><button class="rc-btn" onclick="fd('Didactico_Primaria.zip')">⬇ Descargar ZIP</button></div>
      <div class="rc card"><div class="rc-ico"></div><h4>Presentaciones para el Aula</h4><p>Diapositivas sobre tectónica, Richter y protección. PPT y PDF.</p><button class="rc-btn" onclick="fd('Presentaciones_Aula.zip')">⬇ Descargar</button></div>
      <div class="rc card"><div class="rc-ico"></div><h4>Directorio de Emergencias</h4><p>Números nacionales: COEN, Cruz Roja, Bomberos, MARN, PNC.</p><button class="rc-btn" onclick="fd('Directorio_SV.pdf')">⬇ Descargar PDF</button></div>
...
<section class="sec sec-dark" id="colegio" style="display:none">
  <div class="wrap">
    <div class="school-header-bar" id="schoolHeaderBar">
      <div class="sh-role-badge admin" id="schoolRoleBadge">🛡 Administrador</div>
...
<section class="sec" id="ahora">
  <div class="wrap">
    <div class="sec-hd center">
      <div class="sec-eyebrow" style="background:rgba(230,57,70,.1);border-color:rgba(230,57,70,.25);color:var(--acc2)">🚨 Consulta Rápida · Emergencias</div>
    ...
    <div style="margin-top:20px;background:rgba(230,57,70,.06);border:1px solid rgba(230,57,70,.18);border-radius:var(--r);padding:16px 20px;display:flex;gap:14px;align-items:center;flex-wrap:wrap">
      ...
      <div style="flex:1"><div style="font-weight:700;font-size:.9rem;margin-bottom:4px">Números de emergencia El Salvador</div><div style="display:flex;gap:16px;flex-wrap:wrap;font-size:.82rem;color:var(--text2)"><span>🚑 Cruz Roja: <strong style="color:var(--text)">2222-5155</strong></span><span>🚒 Bomberos: <strong style="color:var(--text)">913</strong></span><span>🚔 PNC: <strong style="color:var(--text)">911</strong></span><span>🌋 MARN: <strong style="color:var(--text)">2267-6000</strong></span><span>🏥 COEN: <strong style="color:var(--text)">2231-4000</strong></span></div></div>
    </div>
  </div>
</section>
